...
\begin{document}
	...
	We consider the linear-nonlinear network architecture shown in Figure \ref{architecture}. \textcolor{red}{shpeal on network architecutre}
	\begin{figure}[ht]
		\centering
		\scalebox{0.8}{
			\begin{tikzpicture}
				...
				\node[main] at (2.5, 4.0) (xi1) {$\xi_1$};
				\node[main] at (2.5, 1.0) (xi2) {$\xi_2$};
				\node[main] at (2.5, -2.0) (xiN) {$\xi_N$};
	...
	\[
		\begin{array}{r@{}}
		N/k \text{ times} ~\left\{\begin{array}{@{}c@{}}\null\\\null\\\null\end{array}\right.\vspace{4pt} \\
		N/k \text{ times}~\left\{\begin{array}{@{}c@{}}\null\\\null\\\null\end{array}\right. \\
		\vspace{7pt}\\ 
		N/k \text{ times}~\left\{\begin{array}{@{}c@{}}\null\\\null\\\null\end{array}\right. \\
		\end{array}
	...
	For simplicity, set $k=1$ for the stimulus weight vector $\mathbf{v}$ and allow $k$ to vary for the noise weight vectors. While increasing $k$ will boost the overall noise added into the neural population, it also changes the direction that the noise is projected into the higher-dimensional neural space. We first consider how the linear Fisher information and mutual information are impacted by the choice of $k$ for the noise weight vector $\mathbf{w}$. Note that both quantities are monotonically increasing functions of $k$, implying that encoding and decoding are enhanced despite the fact that more noise has entered the system. Furthermore, our analytic results ensure linear and logarithmic growth for the Fisher and mutual information, respectively. \textcolor{red}{reference to Brunel/Nadal result}
	...
	While the structured weights provide us analytic results, they possess an unrealistic distribution of synaptic weighting. Thus, we turn to the case of ``unstructured weights,'' in which the synaptic weights are drawn from a lognormal distribution [citations] and thresholded. In this case, we calculate both information theoretic quantities 
